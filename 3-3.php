    ...
    <table class="table table-bordered table-hover" >
      <?php
        if (isset($_SESSION['type'])&&$_SESSION['type']=='管理员') {//不是通过表单到达该页面，需要通过session
          ...
          $sql = "select * from xuanxiang limit $begin_position,$page_size";
          //select * from table limit m,n其中m是指记录开始的index，从0开始，表示第一条记录n是指从第m+1条开始，取n条。
          $result = mysqli_query($conn,$sql);
          $num=mysqli_num_rows($result);
          echo "<table class='table table-bordered table-hover'>";
          echo "<tr><th>编号</th><th>选项申请者</th><th>选项名</th><th>图片</th><th>申请时间</th><th>是否审核</th><th>投票审核</th></tr>";
          ...
          for($i=0;$i<$num;$i++){
              ...
              $schoose = $row['schoose'];
              $simg = $row['simg'];
              $stime=date('Y-m-d',$row['stime']);
              $shenhe= $row['shenhe'];
              // $time=$row['ttime'];
              echo "<tr><td>$id</td><td>$suser</td><td>$schoose</td><td>$simg</td><td>$stime</td><td>$shenhe</td><td>"; 
              echo "<a href='3-3check.php?x=".$row['id']."'>审核</a></td><tr>";
          }
            echo "</table>";   
          echo '<a href="3-3.php?page_number=1">首页</a>  ';
          // for($i=1;$i<=$total_pages;$i++){
          //  echo '<a href="3-3.php?page_number='.$i.'">第'.$i.'页</a>  '; 
          // }
          echo '<a href="3-3.php?page_number='.($current_page_number-1).'">上一页</a>  ';
          echo '<a href="3-3.php?page_number='.($current_page_number+1).'">下一页</a>  ';
          echo '<a href="3-3.php?page_number='.($total_pages).'">尾页</a>  ';
        mysqli_close($conn);
        }else{
